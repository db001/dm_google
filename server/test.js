const res = {
	_events: { finish },
	_eventsCount: 1,
	_maxListeners: undefined,
	outputData: [],
	outputSize: 0,
	writable: true,
	destroyed: false,
	_last: false,
	chunkedEncoding: false,
	shouldKeepAlive: false,
	_defaultKeepAlive: true,
	useChunkedEncodingByDefault: true,
	sendDate: true,
	_removedConnection: false,
	_removedContLen: false,
	_removedTE: false,
	_contentLength: null,
	_hasBody: true,
	_trailer: "",
	finished: false,
	_headerSent: false,
	socket: {
		connecting: false,
		_hadError: false,
		_parent: null,
		_host: null,
		_readableState: {
			objectMode: false,
			highWaterMark: 16384,
			buffer: { head: null, tail: null, length: 0 },
			length: 0,
			pipes: [],
			flowing: true,
			ended: false,
			endEmitted: false,
			reading: true,
			sync: false,
			needReadable: true,
			emittedReadable: false,
			readableListening: false,
			resumeScheduled: false,
			errorEmitted: false,
			emitClose: false,
			autoDestroy: false,
			destroyed: false,
			errored: null,
			closed: false,
			closeEmitted: false,
			defaultEncoding: "utf8",
			awaitDrainWriters: null,
			multiAwaitDrain: false,
			readingMore: false,
			decoder: null,
			encoding: null,
			[Symbol(kPaused)]: false,
		},
		_events: {
			end: [Array],
			timeout: [socketOnTimeout],
			data: [socketOnData],
			error: [socketOnError],
			close: [Array],
			drain: [socketOnDrain],
			resume: [onSocketResume],
			pause: [onSocketPause],
		},
		_eventsCount: 8,
		_maxListeners: undefined,
		_writableState: {
			objectMode: false,
			highWaterMark: 16384,
			finalCalled: false,
			needDrain: false,
			ending: false,
			ended: false,
			finished: false,
			destroyed: false,
			decodeStrings: false,
			defaultEncoding: "utf8",
			length: 0,
			writing: false,
			corked: 0,
			sync: true,
			bufferProcessing: false,
			onwrite: [onwrite],
			writecb: null,
			writelen: 0,
			afterWriteTickInfo: null,
			buffered: [],
			bufferedIndex: 0,
			allBuffers: true,
			allNoop: true,
			pendingcb: 0,
			prefinished: false,
			errorEmitted: false,
			emitClose: false,
			autoDestroy: false,
			errored: null,
			closed: false,
			closeEmitted: false,
		},
		allowHalfOpen: true,
		_sockname: null,
		_pendingData: null,
		_pendingEncoding: "",
		server: {
			maxHeaderSize: undefined,
			insecureHTTPParser: undefined,
			_events: [prototype],
			_eventsCount: 2,
			_maxListeners: undefined,
			_connections: 1,
			_handle: [TCP],
			_usingWorkers: false,
			_workers: [],
			_unref: false,
			allowHalfOpen: true,
			pauseOnConnect: false,
			httpAllowHalfOpen: false,
			timeout: 0,
			keepAliveTimeout: 5000,
			maxHeadersCount: null,
			headersTimeout: 60000,
			requestTimeout: 0,
			_connectionKey: "6::::5000",
			[Symbol(IncomingMessage)]: [IncomingMessage],
			[Symbol(ServerResponse)]: [ServerResponse],
			[Symbol(kCapture)]: false,
			[Symbol(async_id_symbol)]: 6,
		},
		_server: {
			maxHeaderSize: undefined,
			insecureHTTPParser: undefined,
			_events: [prototype],
			_eventsCount: 2,
			_maxListeners: undefined,
			_connections: 1,
			_handle: [TCP],
			_usingWorkers: false,
			_workers: [],
			_unref: false,
			allowHalfOpen: true,
			pauseOnConnect: false,
			httpAllowHalfOpen: false,
			timeout: 0,
			keepAliveTimeout: 5000,
			maxHeadersCount: null,
			headersTimeout: 60000,
			requestTimeout: 0,
			_connectionKey: "6::::5000",
			[Symbol(IncomingMessage)]: [IncomingMessage],
			[Symbol(ServerResponse)]: [ServerResponse],
			[Symbol(kCapture)]: false,
			[Symbol(async_id_symbol)]: 6,
		},
		parser: {
			0: [setRequestTimeout],
			1: [parserOnHeaders],
			2: [parserOnHeadersComplete],
			3: [parserOnBody],
			4: [parserOnMessageComplete],
			5: [onParserExecute],
			6: [onParserTimeout],
			_headers: [],
			_url: "",
			socket: [Circular * 1],
			incoming: [IncomingMessage],
			outgoing: null,
			maxHeaderPairs: 2000,
			_consumed: true,
			onIncoming: [parserOnIncoming],
			[Symbol(resource_symbol)]: [HTTPServerAsyncResource],
		},
		on: [socketListenerWrap],
		addListener: [socketListenerWrap],
		prependListener: [socketListenerWrap],
		_paused: false,
		_httpMessage: [Circular * 2],
		[Symbol(async_id_symbol)]: 10,
		[Symbol(kHandle)]: {
			reading: true,
			onconnection: null,
			_consumed: true,
			[Symbol(owner_symbol)]: [Circular * 1],
		},
		[Symbol(kSetNoDelay)]: false,
		[Symbol(lastWriteQueueSize)]: 0,
		[Symbol(timeout)]: null,
		[Symbol(kBuffer)]: null,
		[Symbol(kBufferCb)]: null,
		[Symbol(kBufferGen)]: null,
		[Symbol(kCapture)]: false,
		[Symbol(kBytesRead)]: 0,
		[Symbol(kBytesWritten)]: 0,
		[Symbol(RequestTimeout)]: undefined,
	},
	_header: null,
	_keepAliveTimeout: 5000,
	_onPendingData: [updateOutgoingData],
	_sent100: false,
	_expect_continue: false,
	req: {
		_readableState: {
			objectMode: false,
			highWaterMark: 16384,
			buffer: { head: null, tail: null, length: 0 },
			length: 0,
			pipes: [],
			flowing: true,
			ended: true,
			endEmitted: true,
			reading: false,
			sync: false,
			needReadable: false,
			emittedReadable: false,
			readableListening: false,
			resumeScheduled: false,
			errorEmitted: false,
			emitClose: true,
			autoDestroy: false,
			destroyed: false,
			errored: null,
			closed: false,
			closeEmitted: false,
			defaultEncoding: "utf8",
			awaitDrainWriters: null,
			multiAwaitDrain: false,
			readingMore: false,
			decoder: null,
			encoding: null,
			[Symbol(kPaused)]: false,
		},
		_events: { end: [clearRequestTimeout] },
		_eventsCount: 1,
		_maxListeners: undefined,
		socket: {
			connecting: false,
			_hadError: false,
			_parent: null,
			_host: null,
			_readableState: [ReadableState],
			_events: [prototype],
			_eventsCount: 8,
			_maxListeners: undefined,
			_writableState: [WritableState],
			allowHalfOpen: true,
			_sockname: null,
			_pendingData: null,
			_pendingEncoding: "",
			server: [Server],
			_server: [Server],
			parser: [HTTPParser],
			on: [socketListenerWrap],
			addListener: [socketListenerWrap],
			prependListener: [socketListenerWrap],
			_paused: false,
			_httpMessage: [Circular * 2],
			[Symbol(async_id_symbol)]: 10,
			[Symbol(kHandle)]: [TCP],
			[Symbol(kSetNoDelay)]: false,
			[Symbol(lastWriteQueueSize)]: 0,
			[Symbol(timeout)]: null,
			[Symbol(kBuffer)]: null,
			[Symbol(kBufferCb)]: null,
			[Symbol(kBufferGen)]: null,
			[Symbol(kCapture)]: false,
			[Symbol(kBytesRead)]: 0,
			[Symbol(kBytesWritten)]: 0,
			[Symbol(RequestTimeout)]: undefined,
		},
		httpVersionMajor: 1,
		httpVersionMinor: 1,
		httpVersion: "1.1",
		complete: true,
		headers: {
			cookie:
				"express:sess=eyJwYXNzcG9ydCI6eyJ1c2VyIjoiZTk2YjJjY2QtZjlhMC00ODg0LWI3NjUtNWJkMTI2NzRhNmNiIn19; express:sess.sig=YcyPqAc6NKbgSDSbmfijQdxLBQQ",
			"accept-language": "en-GB,en-US;q=0.9,en;q=0.8",
			"accept-encoding": "gzip, deflate, br",
			referer:
				"http://localhost:3000/campaigns/cd60c223-ca17-4b8d-8978-882e8678c3b8",
			"sec-fetch-dest": "empty",
			"sec-fetch-mode": "cors",
			"sec-fetch-site": "same-origin",
			origin: "http://localhost:3000",
			"content-type": "application/json",
			"user-agent":
				"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.85 Safari/537.36",
			"sec-ch-ua-mobile": "?0",
			dnt: "1",
			accept: "application/json, text/plain, */*",
			"sec-ch-ua":
				'" Not A;Brand";v="99", "Chromium";v="90", "Google Chrome";v="90"',
			"content-length": "54",
			connection: "close",
			host: "localhost:3000",
		},
		rawHeaders: [
			"cookie",
			"express:sess=eyJwYXNzcG9ydCI6eyJ1c2VyIjoiZTk2YjJjY2QtZjlhMC00ODg0LWI3NjUtNWJkMTI2NzRhNmNiIn19; express:sess.sig=YcyPqAc6NKbgSDSbmfijQdxLBQQ",
			"accept-language",
			"en-GB,en-US;q=0.9,en;q=0.8",
			"accept-encoding",
			"gzip, deflate, br",
			"referer",
			"http://localhost:3000/campaigns/cd60c223-ca17-4b8d-8978-882e8678c3b8",
			"sec-fetch-dest",
			"empty",
			"sec-fetch-mode",
			"cors",
			"sec-fetch-site",
			"same-origin",
			"origin",
			"http://localhost:3000",
			"content-type",
			"application/json",
			"user-agent",
			"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.85 Safari/537.36",
			"sec-ch-ua-mobile",
			"?0",
			"dnt",
			"1",
			"accept",
			"application/json, text/plain, */*",
			"sec-ch-ua",
			'" Not A;Brand";v="99", "Chromium";v="90", "Google Chrome";v="90"',
			"content-length",
			"54",
			"connection",
			"close",
			"host",
			"localhost:3000",
		],
		trailers: {},
		rawTrailers: [],
		aborted: false,
		upgrade: false,
		url: "/api/players/campaign/add/6d3edb1b-4b53-40ea-836c-7e19433c9e57",
		method: "POST",
		statusCode: null,
		statusMessage: null,
		client: {
			connecting: false,
			_hadError: false,
			_parent: null,
			_host: null,
			_readableState: [ReadableState],
			_events: [prototype],
			_eventsCount: 8,
			_maxListeners: undefined,
			_writableState: [WritableState],
			allowHalfOpen: true,
			_sockname: null,
			_pendingData: null,
			_pendingEncoding: "",
			server: [Server],
			_server: [Server],
			parser: [HTTPParser],
			on: [socketListenerWrap],
			addListener: [socketListenerWrap],
			prependListener: [socketListenerWrap],
			_paused: false,
			_httpMessage: [Circular * 2],
			[Symbol(async_id_symbol)]: 10,
			[Symbol(kHandle)]: [TCP],
			[Symbol(kSetNoDelay)]: false,
			[Symbol(lastWriteQueueSize)]: 0,
			[Symbol(timeout)]: null,
			[Symbol(kBuffer)]: null,
			[Symbol(kBufferCb)]: null,
			[Symbol(kBufferGen)]: null,
			[Symbol(kCapture)]: false,
			[Symbol(kBytesRead)]: 0,
			[Symbol(kBytesWritten)]: 0,
			[Symbol(RequestTimeout)]: undefined,
		},
		_consuming: true,
		_dumped: false,
		next: [next],
		baseUrl: "",
		originalUrl:
			"/api/players/campaign/add/6d3edb1b-4b53-40ea-836c-7e19433c9e57",
		_parsedUrl: {
			protocol: null,
			slashes: null,
			auth: null,
			host: null,
			port: null,
			hostname: null,
			hash: null,
			search: null,
			query: null,
			pathname:
				"/api/players/campaign/add/6d3edb1b-4b53-40ea-836c-7e19433c9e57",
			path:
				"/api/players/campaign/add/6d3edb1b-4b53-40ea-836c-7e19433c9e57",
			href:
				"/api/players/campaign/add/6d3edb1b-4b53-40ea-836c-7e19433c9e57",
			_raw:
				"/api/players/campaign/add/6d3edb1b-4b53-40ea-836c-7e19433c9e57",
		},
		params: { id: "6d3edb1b-4b53-40ea-836c-7e19433c9e57" },
		query: {},
		res: [Circular * 2],
		sessionCookies: {
			secure: undefined,
			request: [Circular],
			response: [Circular],
			keys: [Keygrip],
		},
		sessionOptions: {},
		sessionKey: "express:sess",
		session: [Getter / Setter],
		body: { campaign_id: "cd60c223-ca17-4b8d-8978-882e8678c3b8" },
		_body: true,
		length: undefined,
		_passport: { instance: [Authenticator], session: [Object] },
		user: {
			dm_id: "e96b2ccd-f9a0-4884-b765-5bd12674a6cb",
			google_id: "109556704325951156701",
			user_name: "DB",
		},
		route: {
			path: "/api/players/campaign/add/:id",
			stack: [Array],
			methods: [Object],
		},
		[Symbol(kCapture)]: false,
		[Symbol(RequestTimeout)]: undefined,
	},
	locals: {},
	writeHead: [writeHead],
	[Symbol(kCapture)]: false,
	[Symbol(kNeedDrain)]: false,
	[Symbol(corked)]: 0,
	[Symbol(kOutHeaders)]: {
		"x-powered-by": ["X-Powered-By", "Express"],
	},
};
